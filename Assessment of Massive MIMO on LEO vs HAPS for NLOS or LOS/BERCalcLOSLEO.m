...
v_sat = 2; % Satellite velocity in m/s
...
% FOR LOS % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
path_loss = exp(-1i * 2 * pi * f_doppler * (n - 1) / f0) .* ...
            exp(-1i * (n - 1) * 2 * pi * d * cos(theta) / l); % Free space path loss and Doppler shift
H = path_loss / sqrt(N); % Channel matrix of size N x M
